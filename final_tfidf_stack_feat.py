...
for i, (tr, va) in enumerate(StratifiedKFold(y, n_folds=n_folds, random_state=2018)):
    ...
    pac = PassiveAggressiveRegressor(random_state=2018) #设定参数
    pac.fit(trn_term_doc[tr], y[tr]) #训练模型
    score_va = pac.predict(trn_term_doc[va]) #预测
